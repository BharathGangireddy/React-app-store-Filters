...
class AppStore extends Component {
  ...
  getFilteredAppList = searchData => {
    const {activeTabId} = this.state
    const FilteredAppList = searchData.filter(
  ...
  getSearchResults = () => {
    ...
    const searchData = appsList.filter(eachList =>
  ...
  render() {
    ...
    const searchData = this.getSearchResults()
    const FilteredAppList = this.getFilteredAppList(searchData)
      ...
      <div className="app-container ">
        ...
        <ul className="list-container">
          {FilteredAppList.map(eachItem => (
            <AppItem key={eachItem.appId} updateAppDetails={eachItem} />
          ))}
        </ul>
      </div>
    )
